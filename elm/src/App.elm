...
synopticoSetDecoder =
    Decode.succeed SynopticoSet
        |> DP.required "webviews" (Decode.list webViewDecoder)
        |> DP.required "name" Decode.string


webViewDecoder =
    Decode.succeed WebView
        |> DP.required "urls" (Decode.list Decode.string)
        |> DP.required "position" screenPositionDecoder
        |> DP.optional "timer" (Decode.nullable Decode.int) Nothing


screenPositionDecoder =
    Decode.succeed ScreenPosition
        |> DP.required "top" Decode.string
        |> DP.required "left" Decode.string
        |> DP.required "height" Decode.string
        |> DP.required "width" Decode.string
        |> DP.required "zIndex" Decode.string
